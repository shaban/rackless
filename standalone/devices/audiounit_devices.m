...
char* getAudioInputDevices(void) {
    @autoreleasepool {
        ...
        for (NSDictionary *inputDevice in inputDevices) {
            ...
            // Check if device is online/alive - use multiple methods for reliability
            BOOL online = YES; // Default to online
            
            // Method 1: Check if device is alive
            propertyAddress.mSelector = kAudioDevicePropertyDeviceIsAlive;
            propertyAddress.mScope = kAudioObjectPropertyScopeGlobal;
            UInt32 isAlive = 1; // Default to alive
            dataSize = sizeof(UInt32);
            status = AudioObjectGetPropertyData(deviceID, &propertyAddress, 0, NULL, &dataSize, &isAlive);
            if (status == noErr) {
                online = (isAlive != 0);
                NSLog(@"🔍 Input device %u kAudioDevicePropertyDeviceIsAlive: %u (online: %s)", (unsigned int)deviceID, (unsigned int)isAlive, online ? "YES" : "NO");
            } else {
                NSLog(@"⚠️  Input device %u: kAudioDevicePropertyDeviceIsAlive not available (status: %d), assuming online", (unsigned int)deviceID, (int)status);
            }
            
            // Method 2: Try to get current sample rate as a connectivity test
            if (online) {
                propertyAddress.mSelector = kAudioDevicePropertyNominalSampleRate;
                propertyAddress.mScope = kAudioObjectPropertyScopeGlobal;
                Float64 currentSampleRate = 0;
                dataSize = sizeof(Float64);
                OSStatus sampleRateStatus = AudioObjectGetPropertyData(deviceID, &propertyAddress, 0, NULL, &dataSize, &currentSampleRate);
                if (sampleRateStatus != noErr) {
                    NSLog(@"⚠️  Input device %u: Cannot read sample rate (status: %d), might be offline", (unsigned int)deviceID, (int)sampleRateStatus);
                    // Don't mark as offline just for this - some devices might not support this query
                } else {
                    NSLog(@"✅ Input device %u current sample rate: %.0f Hz", (unsigned int)deviceID, currentSampleRate);
                }
            }
            
            NSLog(@"🔍 Input device %u final online status: %s", (unsigned int)deviceID, online ? "YES" : "NO");
...
char* getAudioOutputDevices(void) {
    @autoreleasepool {
        ...
        for (NSDictionary *outputDevice in outputDevices) {
            ...
            if (status == noErr) {
                online = (isAlive != 0);
                NSLog(@"🔍 Output device %u kAudioDevicePropertyDeviceIsAlive: %u (online: %s)", (unsigned int)deviceID, (unsigned int)isAlive, online ? "YES" : "NO");
            } else {
                NSLog(@"⚠️  Output device %u: kAudioDevicePropertyDeviceIsAlive not available (status: %d), assuming online", (unsigned int)deviceID, (int)status);
            }
            ...
            if (online) {
                ...
                OSStatus sampleRateStatus = AudioObjectGetPropertyData(deviceID, &propertyAddress, 0, NULL, &dataSize, &currentSampleRate);
                if (sampleRateStatus != noErr) {
                    NSLog(@"⚠️  Output device %u: Cannot read sample rate (status: %d), might be offline", (unsigned int)deviceID, (int)sampleRateStatus);
                    // Don't mark as offline just for this - some devices might not support this query
                } else {
                    NSLog(@"✅ Output device %u current sample rate: %.0f Hz", (unsigned int)deviceID, currentSampleRate);
                }
            }
            
            NSLog(@"🔍 Output device %u final online status: %s", (unsigned int)deviceID, online ? "YES" : "NO");
